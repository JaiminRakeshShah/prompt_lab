...
def _escape_untrusted(text: str) -> str:
    ...
    escaped = text
    for marker in (DOCUMENT_MARKER_CLOSE, CUSTOMER_MARKER_CLOSE):
...
def render_user(
    ...
    Untrusted customer text is injected only through ``document_text``.
    Named placeholders are filled without ``str.format()``, so literal JSON
    braces in examples stay intact. Missing placeholders raise.
    """
    ...
    return _PLACEHOLDER.sub(lambda match: supplied[match.group(1)], template.user_template)
